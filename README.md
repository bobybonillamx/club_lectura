...
## Actualizar rama sin romper entorno

Si solo quieres **actualizar** tu rama y minimizar conflictos:

```bash
./scripts/actualizar.sh main
```

Este script:
- configura git (`pull.rebase`, `rebase.autoStash`, `fetch.prune`)
- hace `fetch`
- hace `rebase` de tu rama actual sobre `origin/main`

Si aún hay conflicto, tendrás que resolverlo una sola vez y continuar:
